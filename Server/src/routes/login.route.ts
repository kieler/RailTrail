...
export class LoginRoute {
    ...
    private login = async (req: Request, res: Response) => {
        ...
        logger.info(
            `User with username: ${authData?.username} tries logging in.`
        )
        if (
            !authData ||
            !v.validate(authData, AuthenticationRequestSchemaWebsite).valid
        ) {
            res.sendStatus(400)
            return
        }
        ...
        if (!token) {
            ...
            logger.warn(
                `Login for user with username ${authData.username} was not successful`
            )
            res.sendStatus(401)
            return
        }
