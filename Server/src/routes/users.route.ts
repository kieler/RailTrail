...
export class UsersRoute {
    ...
    private constructor() {
        ...
        this.router.post("/password", authenticateJWT, jsonParser, this.changePassword)
    ...
    private async addNewUser(req: Request, res: Response): Promise<void> {
        ...
        if (!userData //||!validateSchema(userData, AuthenticationRequestSchemaWebsite)
        ) {
            res.sendStatus(400)
            return
        }
    ...
    private async changePassword(req: Request, res: Response): Promise<void> {
        ...
        if (!userData //|| !validateSchema(userData, PasswordChangeSchemaWebsite
        ) {
            res.sendStatus(400)
            return
        }
