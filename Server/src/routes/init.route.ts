...
export class InitRoute {
    ...
    private constructor() {
        ...
        this.router.get("/website", authenticateJWT, (req, res) => {
            return this.getAllTracks(req, res)
        })
        this.router.get("/website/:trackId", authenticateJWT, (req, res) => {
            return this.getForTrackWebsite(req, res)
        })
    }
    ...
    private async getForTrack(req: Request, res: Response): Promise<void> {
        ...
        const path: GeoJSON.GeoJSON = await TrackService.getTrackAsLineString(
            track
        )
        ...
        const apiPois: PointOfInterestApp[] | null =
            await this.getAppPoisFromDbPoi(pois)
     ...
     * @returns Nothing
     */
    private async getTrackByPosition(
        req: Request,
        res: Response
    ): Promise<void> {
        ...
        const currentTrack: Track | null =
            await VehicleService.getCurrentTrackForVehicle(backendPos)

        if (!currentTrack) {
            logger.error(
                `Could not find current track with position {lat : ${pos.lat}, lng : ${pos.lng}}`
            )
            res.sendStatus(500)
            return
        }

        const length: number | null = await TrackService.getTrackLength(
            currentTrack
        )

        if (!length) {
            logger.error(
                `Length of track with id ${currentTrack.uid} could not be determined`
            )
            res.sendStatus(500)
            return
        }
     ...
     * @returns Nothing
     */
    private async getForTrackWebsite(
        req: Request,
        res: Response
    ): Promise<void> {
     ...
     */
    private async getAppPoisFromDbPoi(
        pois: POI[]
    ): Promise<PointOfInterestApp[] | null> {
        const apiPois: PointOfInterestApp[] = []
        for (const poi of pois) {
            const type: POIType = poi.typeId
            if (!type) {
                logger.error(
                    `Could not determine type of poi with id ${poi.uid}`
                )
                return null
            }
            const pos: PositionApp = { lat: -1, lng: -1 } // TODO: Do something with the position poi.position.
            const percentagePosition: number | null =
                await POIService.getPOITrackDistancePercentage(poi)
            if (!percentagePosition) {
                logger.error(
                    `Could not determine percentage position of poi with id ${poi.uid}`
                )
                return null
            }
     ...
     */
    private async getWebsitePoisFromDbPoi(
        pois: POI[]
    ): Promise<PointOfInterestWebsite[] | null> {
