...
export class VehicleRoute {
    ...
    private constructor() {
        ...
        this.router.get(
            "/website/crudlist/:trackId",
            authenticateJWT,
            this.getVehicleListCrud
        )
        this.router.post(
            "/website/:trackId",
            authenticateJWT,
            jsonParser,
            this.updateVehicleCrud
        )
        this.router.delete(
            "/website/:vehicleId",
            authenticateJWT,
    ...
    private async updateVehicle(req: Request, res: Response): Promise<void> {
        ...
        if (!userData || !v.validate(userData, UpdateRequestSchemaApp).valid) {
            res.sendStatus(400)
            return
        }

        if (userData.pos) {
            const userVehicle: Vehicle | null =
                await VehicleService.getVehicleById(userData.vehicleId)
            if (!userVehicle) {
                logger.error(
                    `Could not find vehicle with id ${userData.vehicleId}`
                )
                res.sendStatus(500)
                return
            }
            const pos: Feature<Point, GeoJsonProperties> | null =
                await VehicleService.getVehiclePosition(userVehicle)
            if (!pos) {
                logger.error(
                    `Could not find position of vehicle with id ${userVehicle.uid}`
                )
                res.sendStatus(500)
                return
            }
            const position: PositionApp = {
                lat: pos.geometry.coordinates[0],
                lng: pos.geometry.coordinates[1]
            }
            const heading: number = await VehicleService.getVehicleHeading(
                userVehicle
            )
            ...
            const nearbys: Vehicle[] | null =
                await VehicleService.getNearbyVehicles(userVehicle)
            const list: VehicleApp[] = []
            if (nearbys) {
                for (const nearby of nearbys) {
                    const po: Feature<Point, GeoJsonProperties> | null =
                        await VehicleService.getVehiclePosition(nearby)
                    const percentage: number | null =
                        await VehicleService.getVehicleTrackDistancePercentage(
                            nearby
                        )
                    const ve: VehicleApp = {
                        id: nearby.uid,
                        pos: {
                            lat: po?.geometry.coordinates[0]
                                ? po?.geometry.coordinates[0]
                                : 0,
                            lng: po?.geometry.coordinates[1]
                                ? po?.geometry.coordinates[1]
                                : 0
                        },
            ...
            const percentagePositionOnTrack: number | null =
                await VehicleService.getVehicleTrackDistancePercentage(
                    userVehicle
                )
            if (!percentagePositionOnTrack) {
                logger.error(
                    `Could not determine percentage position on track for vehicle with id ${userVehicle.uid}`
                )
                res.sendStatus(500)
                return
            }
    ...
    private async getVehicleList(req: Request, res: Response): Promise<void> {
        ...
        const vehicles: Vehicle[] = await VehicleService.getAllVehiclesForTrack(
            track
        )
        const ret: VehicleWebsite[] = []
        for (const vehicle of vehicles) {
            const pos: Feature<Point, GeoJsonProperties> | null =
                await VehicleService.getVehiclePosition(vehicle)
            if (!pos) {
                logger.error(
                    `Could not find position of vehicle with id ${vehicle.uid}`
                )
                res.sendStatus(500)
                return
            }
            const actualPos: PositionWebsite = {
                lat: pos.geometry.coordinates[0],
                lng: pos.geometry.coordinates[1]
            }
            const heading: number | null =
                await VehicleService.getVehicleHeading(vehicle)
            if (!heading) {
                logger.error(
                    `Could not find heading of vehicle with id ${vehicle.uid}`
                )
                res.sendStatus(500)
                return
            }
    ...
    private async getUid(req: Request, res: Response): Promise<void> {
        ...
        const trackId: number = parseInt(req.params.trackId)
        if (!userData || !v.validate(userData, GetUidSchema).valid) {
            res.sendStatus(400)
            return
        }
        const track: Track | null = await TrackService.getTrackById(trackId)
     ...
     * @returns Nothing
     */
    private async getVehicleListCrud(
        req: Request,
        res: Response
    ): Promise<void> {
        const trackId: number = parseInt(req.params.trackId)
        ...
        const ret: VehicleListItemWebsite[] = await Promise.all(
            (
                ...
                const r: VehicleListItemWebsite = {
                    ...
                    typeId: x.typeId,
                    trackerIds: (
                        await TrackerService.getTrackerByVehicle(x.uid)
                    ).map(y => y.uid)
                }
                return r
            })
     ...
     * @returns Nothing
     */
    private async updateVehicleCrud(
        req: Request,
        res: Response
    ): Promise<void> {
        const trackId: number = parseInt(req.params.trackId)
        const userData: VehicleCrUWebsite = req.body
        if (!userData || !v.validate(userData, VehicleCrUSchemaWebsite).valid) {
            res.sendStatus(400)
            return
        }
        ...
        if (userData.uid) {
            var vehicleToUpdate: Vehicle | null =
                await VehicleService.getVehicleById(userData.uid)
            if (!vehicleToUpdate) {
                logger.error(
                    `Could not find vehicle to update with id ${userData.uid}`
                )
                res.sendStatus(500)
                return
            }

            vehicleToUpdate = await VehicleService.renameVehicle(
                vehicleToUpdate,
                userData.name
            )
            ...
            const type: VehicleType | null =
                await VehicleService.getVehicleTypeById(userData.typeId)
            if (!type) {
                logger.error(
                    `Could not find vehicle type with id ${userData.typeId}`
                )
                res.sendStatus(500)
                return
            }

            vehicleToUpdate = await VehicleService.setVehicleType(
                vehicleToUpdate,
                type
            )
            ...
            if (!vehicleToUpdate) {
                logger.error(
                    `Could not set vehicle type for vehicle with typeid ${userData.typeId}`
                )
                res.sendStatus(500)
                return
            }
            ...
            if (userData.trackerIds && userData.trackerIds.length > 0) {
                for (const trackerId of userData.trackerIds) {
                    const tracker: Tracker | null =
                        await TrackerService.getTrackerById(trackerId)

                    if (!tracker) {
                        logger.error(
                            `Could not find tracker with id ${trackerId}`
                        )
                        res.sendStatus(500)
                        return
                    }
                    vehicleToUpdate =
                        await VehicleService.assignTrackerToVehicle(
                            vehicleToUpdate,
                            tracker
                        )
                    ...
                    if (!vehicleToUpdate) {
                        logger.error(
                            `Could not set tracker with tracker-id ${trackerId}`
                        )
                        res.sendStatus(500)
                        return
                    }
            ...
            const tracker: Tracker | null =
                userData.trackerIds && userData.trackerIds.length > 0
                    ? await TrackerService.getTrackerById(
                          userData.trackerIds[0]
                      )
                    : null // TODO: The createVehicle will probably change

            // TODO: Add physicalName
            const vehicle: Vehicle | null = await VehicleService.createVehicle(
                type,
