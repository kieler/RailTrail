...
export default class VehicleService {
    ...
    public static async createVehicle(
        ...
        return database.vehicles.save(
            type.uid,
            name == null ? undefined : name.trim()
        )
    }
    ...
    public static async getNearbyVehicles(
        ...
        if ((<Vehicle>point).uid) {
            const vehiclePosition = await this.getVehiclePosition(
                <Vehicle>point
            )
        ...
        if (heading != null) {
            ...
            allVehiclesOnTrack.filter(async function (
                vehicle,
                index,
                vehicles
            ) {
                const vehicleTrackKm =
                    await VehicleService.getVehicleTrackDistanceKm(
                        vehicle,
                        track
                    )
        ...
        if (maxDistance != null) {
            allVehiclesOnTrack.filter(async function (
                ...
                const vehicleTrackPosition =
                    await VehicleService.getVehicleTrackPosition(vehicle, track)
                if (
                    vehicleTrackPosition == null ||
                    vehicleTrackPosition.properties == null ||
                    vehicleTrackPosition.properties["trackKm"] == null
                ) {
                    return false
                }
                return (
                    Math.abs(
                        vehicleTrackPosition.properties["trackKm"] -
                            trackDistance
                    ) < maxDistance
                )
            })
        ...
        let vehiclesWithDistances: [Vehicle, number][] = await Promise.all(
            allVehiclesOnTrack.map(async function (vehicle) {
                let vehicleDistance =
                    await VehicleService.getVehicleTrackDistanceKm(vehicle)
     ...
     */
    public static async getAllVehiclesForTrack(
        track: Track,
        type?: VehicleType
    ): Promise<Vehicle[]> {
        ...
        vehicles.filter(async function (vehicle, index, vehicles) {
            const currentTrack = await VehicleService.getCurrentTrackForVehicle(
                vehicle
            )
    ...
    public static async getVehicleTrackPosition(
        ...
        const nearestTrackPointsAndTrack =
            await TrackService.getNearestTrackPoints(vehiclePosition, track)
        if (nearestTrackPointsAndTrack == null) {
            return null
        }
        ...
        // interpolate distance
        const totalDistance =
            distance(trackPoint0, vehiclePosition) +
            distance(trackPoint1, vehiclePosition)
        if (
            trackPoint0.properties == null ||
            trackPoint0.properties["trackKm"] == null
        ) {
        ...
        const vehicleTrackDistance =
            trackPoint0.properties["trackKm"] +
            (distance(trackPoint0, vehiclePosition) / totalDistance) *
                distance(trackPoint0, trackPoint1)
     ...
     */
    public static async getVehicleTrackDistanceKm(
        vehicle: Vehicle,
        track?: Track
    ): Promise<number | null> {
        // get track point of vehicle
        const vehicleTrackPoint = await this.getVehicleTrackPosition(
            vehicle,
            track
        )
        ...
        if (
            vehicleTrackPoint.properties == null ||
            vehicleTrackPoint.properties["trackKm"] == null
        ) {
            return null
        }
     ...
     */
    public static async getVehicleTrackDistancePercentage(
        vehicle: Vehicle,
        track?: Track
    ): Promise<number | null> {
        ...
        const vehicleDistance = await this.getVehicleTrackDistanceKm(
            vehicle,
            track
        )
    ...
    public static async getVehicleTrackHeading(
        ...
        if (nearestTrackPoints.features.length == 1) {
            ...
            const trackPointIndex = trackData.features.indexOf(
                nearestTrackPoints.features[0]
            )
            if (trackPointIndex == 0) {
                nearestTrackPoints.features.push(trackData.features[1])
            } else {
                nearestTrackPoints.features.push(
                    trackData.features[trackPointIndex - 1]
                )
            }
        }
        ...
        if (
            ...
            trackPoint0.properties["trackKm"] >
                trackPoint1.properties["trackKm"]
        ) {
        ...
        if (
            vehicleHeading - trackBearing < 85 ||
            vehicleHeading - trackBearing > -85
        ) {
            return 1
        } else {
     ...
     */
    public static async renameVehicle(
        vehicle: Vehicle,
        newName: string
    ): Promise<Vehicle | null> {
     ...
     */
    public static async setVehicleType(
        vehicle: Vehicle,
        type: VehicleType
    ): Promise<Vehicle | null> {
     ...
     */
    public static async assignTrackerToVehicle(
        vehicle: Vehicle,
        tracker: Tracker
    ): Promise<Vehicle | null> {
     ...
     */
    public static async getVehicleTypeById(
        id: number
    ): Promise<VehicleType | null> {
     ...
     */
    public static async renameVehicleType(
        type: VehicleType,
        newType: string
    ): Promise<VehicleType | null> {
