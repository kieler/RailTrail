...
export default function Login({ dst_url, signup, success }: { dst_url?: Url; signup?: boolean; success?: string }) {
	...
	return success === "true" ? (
		...
		<form action="/webapi/auth" method="POST" className="grid grid-cols-2 gap-y-1 my-1.5 items-center">
			...
			{
				/* display an error message if the login failed */ success === "false" && (
					<div className="col-span-2 bg-red-300 border-red-600 text-black rounded p-2 text-center">
			...
			<button
				type="submit"
				className="col-span-2 rounded-full bg-slate-700 text-white dark:bg-slate-200 dark:text-black mt-1.5">
				Einloggen
			</button>
		</form>
	);
