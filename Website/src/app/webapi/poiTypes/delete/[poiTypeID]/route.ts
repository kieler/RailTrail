...
import {deletePOIType, deleteVehicle, deleteVehicleType} from "@/utils/data";
 ...
 */
export async function DELETE(_request: NextRequest, {params}: {params: {poiTypeID: string}}) {
    // cast the vehicle type to a number
    const poiTypeID = +params.poiTypeID;

    // check if the vehicleTypeID is a number
    if (isNaN(poiTypeID)) {
        console.log('Can not delete vehicle: ',params.poiTypeID, 'is not a Number!');
        return apiError(404);
    }
    ...
    try {
        ...
        const res = await deletePOIType(token, poiTypeID);
        ...
        console.error('Cannot delete poi type', poiTypeID, '- Reason: ', e);
        return apiError(500);
    }
